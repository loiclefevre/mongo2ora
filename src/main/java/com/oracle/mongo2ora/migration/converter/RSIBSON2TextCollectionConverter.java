...
public class RSIBSON2TextCollectionConverter implements Runnable {
	...
	public RSIBSON2TextCollectionConverter(int partitionId, String collectionName, String tableName, CollectionCluster work, CompletableFuture<ConversionInformation> publishingCf, MongoDatabase database, ReactiveStreamsIngestion rsi, ASCIIGUI gui, int batchSize) {
		...
		try {
			...
			try (MongoCursor<RawBsonDocument> cursor = collection.find(Filters.and(
				...
				while (cursor.hasNext()) {
					...
					final byte[] osonData = json.getBytes(StandardCharsets.UTF_8);
